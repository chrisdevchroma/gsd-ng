 ...
 *
 * For local installs (pathPrefix is a relative path like "./.claude/"):
 *   - When `absoluteProjectRoot` is a non-empty string: return a literal
 *     absolute path (e.g. "/home/user/proj/.claude/"). This avoids the
 *     `${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}` shim, which
 *     Claude Code's expansion guard always denies (the guard fires on $()
 *     and ${VAR:-default} regardless of allowlist). Trade-off: deployed copies
 *     are not portable across checkouts — acceptable because deployed copies
 *     live inside the project tree and are regenerated by re-running the
 *     installer.
 *   - When `absoluteProjectRoot` is null/undefined: fall back to the legacy
 *     shim form. This makes the new behavior a feature flag — callers that
 *     do not pass `absoluteProjectRoot` see the old emit unchanged.
 */
function toHomePrefix(pathPrefix, absoluteProjectRoot) {
  ...
  if (dirMatch) {
    if (absoluteProjectRoot) {
      // Path-portability: emit a plain $CLAUDE_PROJECT_DIR-based path.
      // Hook-safe (the bash-safety hook only
      // flags ${VAR:-default} operator forms — plain $VAR is allowed) AND
      // portable (one install works across worktrees, checkouts, colleague
      // machines without re-installing). Trade-off: bash blocks only run
      // when $CLAUDE_PROJECT_DIR is set — which Claude Code always does when
      // invoking Bash tools.
      // The `absoluteProjectRoot` parameter is preserved for API
      // compatibility (callers still distinguish local vs global), but the
      // value itself is no longer baked into the emit.
      return '$CLAUDE_PROJECT_DIR/' + dirMatch[1];
    }
    // Fallback: keep the legacy shim form for any caller that did not pass
    // absoluteProjectRoot. Acts as a feature-flag fallback so this change is
    // not a breaking API for legacy callers.
 ...
 * @param {string} [absoluteProjectRoot] - Absolute project root for local
 *   installs. When provided, toHomePrefix() bakes a literal absolute path
 *   into deployed `.md` files instead of emitting the legacy
 *   `${CLAUDE_PROJECT_DIR:-$(...)}` shim, which Claude Code's expansion
 *   guard would deny. Pass `null` for global installs (the global branches
 *   in toHomePrefix never reach the shim path).
 */
function copyWithPathReplacement(srcDir, destDir, pathPrefix, isCommand = false, absoluteProjectRoot) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      copyWithPathReplacement(srcPath, destPath, pathPrefix, isCommand, absoluteProjectRoot);
      ...
      content = content.replace(globalClaudeRegex, toHomePrefix(pathPrefix, absoluteProjectRoot));
      content = content.replace(globalClaudeHomeRegex, toHomePrefix(pathPrefix, absoluteProjectRoot));
      // Path-portability: for local installs with a known absoluteProjectRoot,
      // emit plain `$CLAUDE_PROJECT_DIR/<dirName>/`
      // instead of baking the literal absolute project root. Hook-safe
      // (plain `$VAR` is allowed by the bash-safety hook — only operator
      // forms like `${VAR:-default}` are denied) AND portable (one install
      // works across worktrees, checkouts, colleague machines without
      // re-installing). See toHomePrefix() for the same change in the
      // tilde/HOME branch.
      // For global installs, fall back to the dirName-only prefix —
      // workflows authored with `./.claude/` are not commonly deployed
      // globally, but this preserves prior semantics.
      if (absoluteProjectRoot) {
        // The JS template literal `$CLAUDE_PROJECT_DIR/${dirName}/` writes
        // the literal sequence `$CLAUDE_PROJECT_DIR/` followed by the
        // JS-evaluated dirName variable's value. JS only interpolates
        // `${...}` (curly-braced), not `$...` — so the output is e.g.
        // `$CLAUDE_PROJECT_DIR/.claude/` which bash expands at runtime.
        content = content.replace(localClaudeRegex, `$CLAUDE_PROJECT_DIR/${dirName}/`);
      } else {
...
function install(isGlobal) {
  ...
  // For local installs, capture the absolute project root so toHomePrefix()
  // can bake it as a literal absolute path into deployed `.md` files. Without
  // this, the deployed copies would still contain the legacy
  // `${CLAUDE_PROJECT_DIR:-$(...)}` shim — which Claude Code's expansion
  // guard denies on every Bash tool call. For global installs we leave this
  // null; the global branches in toHomePrefix never reach the shim path.
  const absoluteProjectRoot = isGlobal ? null : process.cwd();
  ...
  if (isClaudeCode) {
    ...
    copyWithPathReplacement(gsdSrc, gsdDest, pathPrefix, true, absoluteProjectRoot);
    ...
    copyWithPathReplacement(skillSrc, skillDest, pathPrefix, false, absoluteProjectRoot);
    ...
    if (fs.existsSync(agentsSrc)) {
      ...
      for (const entry of agentEntries) {
        if (entry.isFile() && entry.name.endsWith('.md')) {
          ...
          content = content.replace(dirRegex, toHomePrefix(pathPrefix, absoluteProjectRoot));
          content = content.replace(homeDirRegex, toHomePrefix(pathPrefix, absoluteProjectRoot));
          content = processAttribution(content, getCommitAttribution());
