...
describe('commands.cjs branch coverage residuals (60-11)', () => {
  ...
  describe('cmdDivergence branches', () => {
    test('triage rejects invalid status', () => {
      ...
      const code = `require('${path.resolve('/home/chris/Development/gsd-workspace/gsd-ng/gsd-ng/bin/lib/commands.cjs')}').cmdDivergence(${JSON.stringify(tmpDir)}, { triage: 'abc1234', status: 'BOGUS', reason: 'x' });`;
      const r =
    ...
    test('commit_docs=false short-circuits with skipped', () => {
      ...
      try {
        ...
        const code =
          ...
          JSON.stringify(
            path.resolve(
              '/home/chris/Development/gsd-workspace/gsd-ng/gsd-ng/bin/lib/commands.cjs',
            ),
          ) +
          ').cmdCommit(' +
          JSON.stringify(proj) +
