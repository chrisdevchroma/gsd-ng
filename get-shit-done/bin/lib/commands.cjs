 ...
 */
function writeDivergenceFile(filePath, upstreamUrl, commits) {
  const today = new Date().toISOString().split('T')[0];
  const rows = commits.map(c =>
 ...
 *
 * @param {string} cwd
 * @param {{ refresh?: boolean, init?: boolean, triage?: string, status?: string, reason?: string, branch?: string, base?: string }} opts
 * @param {boolean} raw
 */
function cmdDivergence(cwd, opts, raw) {
  ...
  try {
    upstreamUrl = execSync('git remote get-url upstream', { cwd, encoding: 'utf8', stdio: 'pipe' }).trim();
  } catch {}

  if (!upstreamUrl) {
    output({ status: 'no_upstream', commits: [] }, raw, 'No upstream remote found. Add one with: git remote add upstream <url>');
    return;
  }
  ...
  if (opts.refresh) {
    try {
      execSync('git fetch upstream', { cwd, stdio: 'pipe', timeout: 30000 });
    } catch {
  ...
  if (opts.init) {
    let mergeBase = '';
    try {
      mergeBase = execSync('git merge-base HEAD upstream/main', { cwd, encoding: 'utf8', stdio: 'pipe' }).trim();
    } catch {}

    const range = mergeBase ? `${mergeBase}..upstream/main` : 'upstream/main';
    let allCommits = [];
    try {
      const log = execSync(
    ...
    writeDivergenceFile(divergencePath, upstreamUrl, allCommits);
  ...
  try {
    const log = execSync(
      'git log HEAD..upstream/main --format="%H|%ad|%s" --date=short',
      { cwd, encoding: 'utf8', stdio: 'pipe' }
    ).trim();
    if (log) {
      commits = log.split('\n').map(line => {
...
function detectInstallLocation(cwd) {
  ...
  const localPath = path.join(cwd, '.claude', 'get-shit-done', 'VERSION');
  const globalPath = path.join(homeDir, '.claude', 'get-shit-done', 'VERSION');
...
function cmdUpdate(cwd, options, raw, _testOverrides) {
  ...
  if (!installInfo) {
    return output({
      ...
      message: 'No VERSION file found. Run npx get-shit-done-ng@latest to install.',
    }, raw);
  }
  ...
  if (_testOverrides) {
    ...
    // Try npm first
    try {
      const npmResult = execSync('npm view get-shit-done-ng version', {
        timeout: 15000,
    ...
    if (!latestVersion) {
      const githubScript = `
        ...
        const opts = {
          ...
          path: '/repos/chrisdevchroma/get-shit-done-ng/releases/latest',
          headers: { 'User-Agent': 'get-shit-done-ng', 'Accept': 'application/vnd.github.v3+json' },
          timeout: 10000,
        };
  ...
  if (process.env.GSD_TEST_DRY_EXECUTE) {
    let installCommand;
    if (updateSource === 'npm') {
      installCommand = `npx -y get-shit-done-ng@latest ${installFlag}`;
    } else {
  ...
  if (updateSource === 'npm') {
    try {
      execSync(`npx -y get-shit-done-ng@latest ${installFlag}`, {
        stdio: 'inherit',
        timeout: 120000,
      });
    } catch (e) {
    ...
    const tarballPath = path.join(tmpExtractDir, 'get-shit-done-ng.tar.gz');
    const extractDir = path.join(tmpExtractDir, 'extracted');
    const tarballUrl = `https://github.com/chrisdevchroma/get-shit-done-ng/releases/download/${latestVersion}/get-shit-done-ng.tar.gz`;
    ...
    try {
      ...
      const downloadScript = `
        ...
        function download(url, dest, cb) {
          ...
          const req = mod.get(url, { headers: { 'User-Agent': 'get-shit-done-ng' }, timeout: 30000 }, function(res) {
