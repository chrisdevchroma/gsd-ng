...
```bash
# Flag parsing
PR_DRAFT=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" config-get git.pr_draft --raw 2>/dev/null || echo "true")
...
```bash
PLATFORM=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" detect-platform "$REMOTE" --field platform --raw 2>/dev/null || echo "")
CLI=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" detect-platform "$REMOTE" --field cli --raw 2>/dev/null || echo "")
CLI_INSTALLED=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" detect-platform "$REMOTE" --field cli_installed --raw 2>/dev/null || echo "false")
PLATFORM_SOURCE=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" detect-platform "$REMOTE" --field source --raw 2>/dev/null || echo "unknown")
CLI_INSTALL_URL=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" detect-platform "$REMOTE" --field cli_install_url --raw 2>/dev/null || echo "")
```
...
TYPE_ALIAS=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-type-alias "${TYPE:-feat}" --raw 2>/dev/null || echo "${TYPE:-feature}")
```
...
for summary in $(ls .planning/phases/${PHASE_DIR_NAME}/*-SUMMARY.md 2>/dev/null | sort); do
  ONE_LINER=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" summary-extract "$summary" --fields one_liner --raw 2>/dev/null || echo "")
  if [ -n "$ONE_LINER" ]; then
...
PR_TEMPLATE_PATH=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" config-get git.pr_template --raw 2>/dev/null || echo "")
...
---
*Generated by [GSD-NG](https://github.com/chrisdevchroma/get-shit-done-ng) from phase ${PHASE_NUMBER} execution*
PRTEMPLATE
fi
```
