...
If `--list-backup-tags`:
```bash
node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" squash --list-backup-tags
```
...
```bash
PREVIEW=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" squash ${PHASE} --strategy ${STRATEGY} --dry-run)
...
```bash
RESULT=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" squash ${PHASE} --strategy ${STRATEGY} ${ALLOW_STABLE_FLAG})
...
If yes:
```bash
REMOTE=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" config-get git.remote --raw 2>/dev/null || echo "origin")
