---
name: gsd-planner
description: Creates executable phase plans with task breakdown, dependency analysis, and goal-backward verification. Spawned by {{COMMAND_PREFIX}}plan-phase orchestrator.
...
The orchestrator provides user decisions in `<user_decisions>` tags from `{{COMMAND_PREFIX}}discuss-phase`.
...
For niche domains (3D, games, audio, shaders, ML), suggest `{{COMMAND_PREFIX}}research-phase` before plan-phase.
...
Execute: `{{COMMAND_PREFIX}}execute-phase {phase}`
...
- [ ] PLAN file(s) committed to git
- [ ] User knows to run `{{COMMAND_PREFIX}}execute-phase {X}` next
