...
3. In Claude Code, consider adding sensitive files to "Deny" permissions (other harnesses: use the equivalent deny/allowlist mechanism)
...
```bash
node "$HOME/.claude/gsd-ng/bin/gsd-tools.cjs" commit "docs: map existing codebase" --files .planning/codebase/*.md
```
