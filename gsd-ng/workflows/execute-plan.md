...
@~/.claude/gsd-ng/references/git-integration.md
</required_reading>
...
See `~/.claude/gsd-ng/references/tdd.md` for structure.
</tdd_plan_execution>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/gsd-ng/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
node "$HOME/.claude/gsd-ng/bin/gsd-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
node "$HOME/.claude/gsd-ng/bin/gsd-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
