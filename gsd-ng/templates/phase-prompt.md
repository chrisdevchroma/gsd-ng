...
**Wave is pre-computed:** Wave numbers are assigned during `{{COMMAND_PREFIX}}plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/gsd-ng/references/tdd.md` for TDD plan structure.
...
<!-- Checkpoint pattern: the agent starts server, user visits URL. See checkpoints-deep.md for full patterns. -->
<task type="auto">
...
See `~/.claude/gsd-ng/templates/user-setup.md` for full schema and examples
...
See `~/.claude/gsd-ng/workflows/verify-phase.md` for verification logic.
