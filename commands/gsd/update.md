...
1. Check for updates:
!`node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" update --dry-run`
...
2. Parse the result:
   ...
   - If status is "unknown_version": tell the user "No GSD installation found. Run: npx get-shit-done-ng@latest" and stop.
   - If status is "both_unavailable": tell the user the message from the result and stop.

3. If update_available: show the user "Update available: {installed} -> {latest} (via {update_source})" and warn about clean install (commands/gsd/ and get-shit-done/ will be wiped and replaced; custom files preserved).
...
!`node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" update --{install_type}`
