...
argument-hint: "[--refresh] [--init] [--triage <hash> --status <status> --reason \"text\"]"
allowed-tools:
  - Read
  - Bash
  - Write
---
<objective>
Show upstream divergence status and manage per-commit triage decisions. Tracks which upstream commits have been picked, skipped, or deferred with rationale.
</objective>
...
```bash
GSD_TOOLS="${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}/.claude/get-shit-done/bin/gsd-tools.cjs"
